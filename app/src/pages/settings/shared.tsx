...
export function SettingsDataBackupSection({ embedded }: { embedded?: boolean } = {}) {
  ...
  useEffect(() => {
    ...
    const openIfBackupHash = () => {
      const raw = window.location.hash.replace(/^#/, "");
      if (raw === "settings-backup" || raw === "data") {
        setBackupExpanded(true);
      }
    };
    openIfBackupHash();
    window.addEventListener("hashchange", openIfBackupHash);
    return () => window.removeEventListener("hashchange", openIfBackupHash);
  }, [embedded]);
  ...
  const backupBody = (
    <>
      <div className="space-y-2.5 rounded-xl border border-border/50 bg-background/40 p-3 dark:bg-background/20">
          ...
          {ALL_BACKUP_SCOPES.map((scope) => (
            <div key={scope} className="flex items-start gap-2.5">
              <Checkbox
                ...
                onCheckedChange={(c) => setExportScopes(new Set(toggleScope(exportScopes, scope, c === true)))}
                className="mt-0.5"
  ...
  return (
    <div className={cn(!embedded && "border-t border-border pt-6")}>
      <div className="overflow-hidden rounded-2xl border border-border/60 bg-gradient-to-b from-muted/30 to-muted/5 shadow-sm ring-1 ring-border/30 dark:from-muted/15 dark:to-muted/5">
        {embedded ? (
          <Collapsible open={backupExpanded} onOpenChange={setBackupExpanded} className="group">
            ...
            <CollapsibleContent className="border-t border-border/50 px-4 pb-4 pt-4 sm:px-5 sm:pb-5">
              {backupBody}
            </CollapsibleContent>
          </Collapsible>
        ) : (
          <div className="p-4 sm:p-5">
            ...
            <div className="mt-4">{backupBody}</div>
          </div>
        )}
      </div>
