...
import { TemporarilyUnavailableError } from "@/lib/fetchWithRetry";
...
export default function AICoach() {
  ...
  const chatMutation = useMutation({
    ...
    onError: (err) => {
      if (err instanceof TemporarilyUnavailableError) {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 relative min-h-[calc(100vh-200px)]">
      ...
      {temporarilyUnavailable && (
        <Card className="border-amber-500/50 bg-amber-50/60 dark:bg-amber-950/30" data-testid="ai-coach-unavailable">
          <CardContent className="p-4 flex items-start justify-between gap-3">
            <div className="min-w-0">
              <p className="text-sm font-medium">Temporarily unavailable</p>
            ...
            <div className="flex items-center gap-2 shrink-0">
              <Button
                ...
                size="sm"
                onClick={() => {
                  if (lastAttempt && !chatMutation.isPending) {
                    chatMutation.mutate(lastAttempt);
                  }
                }}
                disabled={!lastAttempt || chatMutation.isPending}
                data-testid="button-ai-coach-try-again"
              >
                Try again
              </Button>
              <Button
                variant="ghost"
                size="sm"
                onClick={() => setTemporarilyUnavailable(false)}
                data-testid="button-ai-coach-dismiss-unavailable"
              >
                Dismiss
              </Button>
            </div>
