...
function RatioAdviserTool({ settings, bgUnit }: { settings: UserSettings; bgUnit: string }) {
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              {stepLabels.map((label, i) => (
                <span key={i} className={`${i === step ? "font-medium text-foreground" : ""} ${i > step ? "hidden sm:inline" : ""}`}>
                  {i > 0 && i <= step && <span className="mx-1">›</span>}
                  {i <= step && label}
                </span>
              ))}
            </div>
...
export default function Ratios() {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6" data-testid="page-ratios">
      ...
      <RatioAdviserTool settings={settings} bgUnit={bgUnit} />
