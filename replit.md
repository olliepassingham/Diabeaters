...
- **Scenarios**: Bedtime Readiness Check (first tab - calm evening check for stable sleep with save-to-history and 14-day log with pattern insights), Sick Day Mode (with journal/timeline for logging BG, ketones, corrections, fluids, symptoms and BG trend analysis), and Travel Mode (with pre-travel GP appointment reminder) with tabbed interface. Scenario history auto-records past sick days and trips on deactivation. Activity Advisor shows scenario-aware banners and contextual notes when sick day or travel mode is active.
