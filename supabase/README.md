...
4b. **Beatie scheduled feed posts** — deploy **`beatie_feed_post_cron`** (`supabase functions deploy beatie_feed_post_cron --no-verify-jwt`). Requires `BEATIE_FEED_BOT_USER_ID`, `ENABLE_AI_COACH=true`, and `OPENAI_API_KEY`. Schedule daily POST to `…/functions/v1/beatie_feed_post_cron` (service role or **`BEATIE_FEED_POST_CRON_SECRET`** header); the function defaults to at most one post every **72 hours**. See [`docs/operations/beatie_feed_bot_setup.md`](../docs/operations/beatie_feed_bot_setup.md).
