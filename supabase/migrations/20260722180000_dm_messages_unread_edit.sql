-- Allow senders to edit their own unread DM text while the recipient has not read it yet.
...
BEGIN
  ...
  -- Must keep either non-empty text or an attached image (matches dm_messages_body_image_check).
  ...
  -- Refresh unread in-app notification preview so the inbox matches the edited text.
...
COMMENT ON FUNCTION public.edit_unread_dm_message(uuid, text) IS
  'Edits a DM the caller sent, only while the recipient has not read it yet.';
