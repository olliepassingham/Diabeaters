/**
 * CI secrets checker. Fails with a readable checklist if required secrets are missing.
...
const REQUIRED = [
  ...
  { key: "VITE_SUPABASE_URL", where: "Supabase → Project Settings → API" },
  { key: "VITE_SUPABASE_ANON_KEY", where: "Supabase → Project Settings → API" },
];
...
function main() {
  ...
  console.error("\n❌ Missing required secrets. Add them in GitHub → Settings → Secrets and variables → Actions:\n");
  for (const { key, where } of missing) {
    console.error(`  • ${key}`);
    console.error(`    Where to get: ${where}`);
  }
  console.error("\nSee docs/ci_secrets.md for copy-paste instructions.\n");
  process.exit(1);
}
